		...
		<Menu v-tooltip="tooltip" as="div" class="relative inline-block text-left">
			<div>
				<MenuButton
					...
					:style="styles"
				>
					<template v-if="text || leftIcon || rightIcon">
			...
			<transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
				<MenuItems
					:class="`absolute right-0 z-20 mt-2 w-56 origin-top-right bg-white border border-gray-100 py-1.5 shadow-${shadow} focus:outline-none`"
					:style="styles"
				>
					<MenuItem
					...
					>
						<button
							:class="[
								active ? 'bg-gray-100' : '',
								'flex text-left px-4 py-2 w-full',
						...
						>
							<div v-if="item.icon" class="text-xl flex items-center">
								<TwIcon :name="item.icon" class="pr-3 h-max opacity-70"></TwIcon>
							</div>
							{{ item.label }}
						</button>
					</MenuItem>
